    ...
    <section class="min-h-screen bg-gradient-to-br from-indigo-900 to-purple-800 text-white px-4 py-16">
      <div class="max-w-5xl mx-auto space-y-12">
        ...
        <header class="text-center space-y-4">
          <h1 class="text-4xl font-bold text-purple-200">📚 Abstraksi - Somatic</h1>
        ...
        <div class="bg-[#2d1f4a] p-6 rounded-xl shadow space-y-4">
          <h2 class="text-xl font-semibold text-purple-300">🧠 Studi Kasus</h2>
          <p class="text-slate-300">
            Kamu sedang mengembangkan sebuah <strong class="text-purple-200">aplikasi manajemen waktu belajar</strong> untuk siswa.
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          <!-- Sumber Informasi -->
          <div>
            <h3 class="text-lg font-semibold text-purple-300 mb-2">📦 Informasi yang Tersedia</h3>
            <div class="space-y-2">
              <div
          ...
          <!-- Drop Area -->
          <div>
            <h3 class="text-lg font-semibold text-purple-300 mb-2">🎯 Fitur Inti Aplikasi</h3>
            <div
              class="min-h-[200px] bg-purple-950 p-4 rounded-xl border-2 border-dashed border-purple-400"
              @dragover.prevent
              @drop="onDrop"
            >
              <div
                ...
                :key="'dropped-' + index"
                class="bg-purple-800 p-3 rounded-lg mb-2 shadow-md"
              >
                {{ item.text }}
              </div>
              <p v-if="droppedItems.length === 0" class="text-slate-400 italic">Seret item penting ke sini</p>
            </div>
          </div>
        </div>
        ...
        <div class="text-center pt-8 space-y-4">
          <div v-if="showFeedback && isCorrect" class="text-green-300 font-semibold">✅ Bagus! Semua informasi yang kamu pilih relevan.</div>
          <div v-if="showFeedback && !isCorrect" class="text-red-300 font-semibold">❌ Ada informasi yang kurang tepat. Coba lagi.</div>

          <div class="space-x-4">
            <button @click="reset" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded">🔄 Reset</button>
            <button @click="checkAnswer" class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded">✅ Cek Jawaban</button>
            <button
              @click="goToNext"
              :disabled="!isCorrect"
...
<!-- WARNING: methods: section not found -->