    ...
    <section class="min-h-screen bg-gradient-to-br from-indigo-900 to-purple-800 text-white px-4 py-16">
      <div class="max-w-4xl mx-auto space-y-12">
        ...
        <header class="text-center space-y-4">
          <h1 class="text-4xl font-bold text-purple-200">🎧 Abstraksi - Auditory</h1>
          <p class="text-lg text-slate-200">Memahami inti dari permasalahan melalui suara dan penjelasan verbal.</p>
        </header>

        <!-- Penjelasan Audio -->
        <div class="bg-[#261b3f] p-6 rounded-xl space-y-4 shadow-lg">
          <h2 class="text-xl font-semibold text-purple-300">🔊 Penjelasan Audio</h2>
          <audio controls class="w-full rounded">
            <source src="/audio/abstraksi-penjelasan.mp3" type="audio/mpeg" />
        ...
        <div class="bg-[#2d1f4a] p-6 rounded-xl shadow space-y-3">
          <h2 class="text-xl font-semibold text-purple-300">📖 Narasi Teks</h2>
          <p class="text-slate-300">
        ...
        <!-- Quiz Interaktif -->
        <div class="bg-[#261b3f] p-6 rounded-xl space-y-4 shadow-lg">
          <h2 class="text-xl font-semibold text-purple-300">🧪 Kuis Interaktif</h2>
          <p class="text-slate-300">Pilih jawaban yang paling tepat untuk memahami pemahamanmu tentang abstraksi:</p>

          <div class="mt-4 space-y-3">
            <div
              v-for="(option, index) in options"
              :key="index"
              class="border border-purple-600 rounded-lg p-3 cursor-pointer hover:bg-purple-700 transition-all"
              :class="{
                'bg-green-600 text-white': selected === option.value && option.value === 'b',
        ...
        <div class="text-right">
          <button :disabled="!isCorrect" class="mt-4 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-500 text-white font-semibold py-2 px-6 rounded-full transition-all duration-300" @click="goNext">
            Lanjut ke Visual →
          </button>
        </div>
